  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4 }}>
        ...
        {recipes.length > 0 ? (
          <Box>
            {recipes.map((recipe, index) => (
              <Box key={index} sx={{ mb: 2, p: 2, border: '1px solid #ddd', borderRadius: 1 }}>
                <Typography variant="h6">{recipe.product?.name} - {recipe.variant}</Typography>
                <Typography>Ingredients: {recipe.ingredients?.length || 0}</Typography>
              </Box>
            ))}
          </Box>
        ) : (
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography>
          Coffee drinks: {products.filter(p => ['Espresso', 'Coffee'].includes(p.category)).length}
        </Typography>
        <Typography>
          Other products (filtered out): {products.filter(p => !['Espresso', 'Coffee'].includes(p.category)).length}
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
          Note: Only coffee drinks are shown on the menu. Bakery section is a placeholder for future items.
        </Typography>
      </Box>
    </Container>
  );
